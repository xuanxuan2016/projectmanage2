...
namespace Framework\Service\Lib\PhpAmqpLib\Connection;

use Framework\Service\Lib\PhpAmqpLib\Channel\AMQPChannel;
use Framework\Service\Lib\PhpAmqpLib\Channel\AbstractChannel;
use Framework\Service\Lib\PhpAmqpLib\Exception\AMQPProtocolConnectionException;
use Framework\Service\Lib\PhpAmqpLib\Exception\AMQPRuntimeException;
use Framework\Service\Lib\PhpAmqpLib\Exception\AMQPTimeoutException;
use Framework\Service\Lib\PhpAmqpLib\Wire\AMQPReader;
use Framework\Service\Lib\PhpAmqpLib\Wire\AMQPTable;
use Framework\Service\Lib\PhpAmqpLib\Wire\AMQPWriter;
use Framework\Service\Lib\PhpAmqpLib\Wire\IO\AbstractIO;
use Framework\Service\Lib\PhpAmqpLib\Wire\IO\SocketIO;
use Framework\Service\Lib\PhpAmqpLib\Wire\IO\StreamIO;
...
class AbstractConnection extends AbstractChannel {
    ...
    /** @var \Framework\Service\Lib\PhpAmqpLib\Wire\IO\AbstractIO */
    protected $io;
    ...
    $user, $password, $vhost = '/', $insist = false, $login_method = 'AMQPLAIN', $login_response = null, $locale = 'en_US', AbstractIO $io, $heartbeat = 0, $connection_timeout = 0
    ) {
    ...
    public function prepare_content($channel, $class_id, $weight, $body_size, $packed_properties, $body, $pkt = null) {
        ...
        $key_cache = sprintf(
                '%s|%s|%s|%s', $channel, $packed_properties, $class_id, $weight
        );
     ...
     * @return array
     * @throws \Exception
     * @throws \Framework\Service\Lib\PhpAmqpLib\Exception\AMQPTimeoutException
     * @throws \Framework\Service\Lib\PhpAmqpLib\Exception\AMQPRuntimeException
     */
     ...
     * @throws \Framework\Service\Lib\PhpAmqpLib\Exception\AMQPProtocolConnectionException
     */
    ...
    protected function connection_redirect($args) {
        ...
        $this->debug->debug_msg(sprintf(
                        'Redirected to [%s], known_hosts [%s]', $host, $this->known_hosts
        ));
    ...
    protected function connection_start($args) {
        ...
        $this->debug->debug_connection_start(
                $this->version_major, $this->version_minor, $this->server_properties, $this->mechanisms, $this->locales
        );
    }
    ...
    /**
     * @return \Framework\Service\Lib\PhpAmqpLib\Wire\IO\AbstractIO
     */
