 ...
 * @license MIT License
 * */
if (!defined('XLSXWRITER_CLASS')) {
    define('XLSXWRITER_CLASS', true);
	...
	public function writeSheet(array $data, $sheet_name='', array $header_types=array())
	{
		...
		//$this->finalizeSheet($sheet_name); // 这里有close fd 句柄，导致后续的无法再写入当前sheet_name的sheet表
	}
	...
	public static function log($string)
	{
		// file_put_contents("php://stderr", date("Y-m-d H:i:s:").rtrim(is_array($string) ? json_encode($string) : $string)."\n");
                Log::getInstance()->log('XlsxWriter.Class.php:'.rtrim(is_array($string) ? json_encode($string) : $string)."\n", LOG::LOG_ERR);
	}
	//------------------------------------------------------------------
	...
	public static function sanitize_sheetname($sheetname) 
	{
		...
		$sheetname = mb_substr($sheetname, 0, 31); // substr($sheetname, 0, 31);
