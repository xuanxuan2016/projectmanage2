...
namespace Framework\Service\MarkDown;
...
class HyperDown {
    ...
    public function optimizeLines($html) {
        $last = 0;
        return $this->_line ?
                preg_replace_callback("/class=\"line\" data\-start=\"([0-9]+)\" data\-end=\"([0-9]+)\" (data\-id=\"{$this->_uniqid}\")/", function ($matches) use (&$last) {
                    if ($matches[1] != $last) {
    ...
    public function parseInline($text, $whiteList = '', $clearHolders = true, $enableAutoLink = true) {
        ...
        // link
        $text = preg_replace_callback(
                "/<(https?:\/\/.+)>/i", function ($matches) use ($self) {
        ...
        // encode unsafe tags
        $text = preg_replace_callback(
                "/<(\/?)([a-z0-9-]+)(\s+[^>]*)?>/i", function ($matches) use ($self, $whiteList) {
        ...
        $text = preg_replace_callback(
            ...
            $result = isset($self->_definitions[$matches[2]]) ?
                    "<img src=\"{$self->_definitions[$matches[2]]}\" alt=\"{$escaped}\" title=\"{$escaped}\">" : $escaped;
        ...
        $text = preg_replace(
                "/<([_a-z0-9-\.\+]+@[^@]+\.[a-z]{2,})>/i", "<a href=\"mailto:\\1\">\\1</a>", $text
        );
        // autolink url
        if ($enableAutoLink) {
            $text = preg_replace_callback(
                    "/(^|[^\"])((https?):[\p{L}_0-9-\.\/%#!@\?\+=~\|\,&\(\)]+)($|[^\"])/iu", function ($matches) use ($self) {
    ...
    public function parseInlineCallback($text) {
        ...
        $text = preg_replace_callback(
                "/(\s+|^)(_{3})(.+?)\\2(\s+|$)/", function ($matches) use ($self) {
        ...
        $text = preg_replace_callback(
                "/(\s+|^)(_{2})(.+?)\\2(\s+|$)/", function ($matches) use ($self) {
        ...
        $text = preg_replace_callback(
                "/(\s+|^)(_)(.+?)\\2(\s+|$)/", function ($matches) use ($self) {
            return $matches[1] . '<em>' .
                    $self->parseInlineCallback($matches[3]) .
                    '</em>' . $matches[4];
        }, $text
     ...
     * 获取需要解析的块
     *
     * @param string $text 需要解析的文件内容
     * @param array $lines 文件内容的行
     * @return array 返回需要解析的块
     * <br>block内容:['块类型','对应的内容行数','','关键字长度']
     */
    ...
    private function parseBlockList($block, $key, $line, &$state) {
        if (preg_match("/^(\s*)((?:[0-9]+\.)|\-|\+|\*)\s+/i", $line, $matches)) {
            ...
            if ($state['empty'] <= 1 && preg_match("/^(\s+)/", $line, $matches) && strlen($matches[1]) > $block[3]) {
                $state['empty'] = 0;
                $this->setBlock($key);
                return false;
    ...
    private function parseBlockCode($block, $key, $line) {
        if (preg_match("/^(\s*)(~{3,}|`{3,})([^`~]*)$/i", $line, $matches)) {
            if ($this->isBlock('code')) {
                ...
                if ($this->isBlock('list')) {
                    $space = $block[3];
                    $isAfterList = ($space > 0 && strlen($matches[1]) >= $space) || strlen($matches[1]) > $space;
                }
    ...
    private function parseBlockMh($block, $key, $line, &$state, $lines) {
        if (preg_match("/^\s*((=|-){2,})\s*$/", $line, $matches) && ($block && $block[0] == "normal" && !preg_match("/^\s*$/", $lines[$block[2]]))) {    // check if last line isn't empty
        ...
        while (isset($blocks[$key])) {
            ...
            if ('pre' == $type) {
                $isEmpty = array_reduce(
                        array_slice($lines, $block[1], $block[2] - $block[1] + 1), function ($result, $line) {
    ...
    private function isBlock($type, $value = NULL) {
        return $this->_current == $type && (NULL === $value ? true : $this->_blocks[$this->_pos][3] == $value);
    }
